...
async function loadReceiveDocs() {
    loading.value = true;
    try {
        const result = await ReceiveDocService.getReceiveDocListByStatus(searchQuery.value, fromDate.value, toDate.value, 2, currentPage.value, pageSize.value);
...
<template>
    <div class="flex flex-col gap-6">
        <div class="card">
            ...
            <ReceiveDocTable :data="receiveDocs" :loading="loading" :totalRecords="totalRecords" :currentPage="currentPage" :pageSize="pageSize" mode="history" :enableRowClick="true" @page-change="onPageChange" @row-click="viewDetail" />
        </div>
